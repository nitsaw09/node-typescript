...
describe('Teams', () => {  
    ...
    describe('Delete teams', () => {
        it('should delete teams with status code 200', async () => {  
            mockRequest.body = {
                teams: [
                    {
                        name: 'Arsenal',
            ...
            const newTeamData = teamsData.splice(1, 1);
            ...
            mockFs.writeFile.mockImplementationOnce(
                (): Promise<any> => Promise.resolve(newTeamData)
            );

            await deleteTeams(mockRequest as Request, mockResponse as Response);
            expect(mockResponse.statusCode).toBe(200);
            expect(responseData).toEqual(expectedResponse);
        });
